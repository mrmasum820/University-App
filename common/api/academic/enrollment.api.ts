...
export const EnrollmentQueryKey = {
  ALL_List: "enrollmentALLList",
  List: "enrollmentList",
...
//  api config
export const enrollmentAllList = async <R>(params: paramsProps): Promise<R> => {
...
export const enrollmentCreate = async <T, R>(data: T): Promise<R> => {
  const res = await apiConfig.post<R>(ApiEndPoint.CREATE_API, data);
  return res.data;
};

export const enrollmentEdit = async (id: number) => {
  const res = await apiConfig.get(ApiEndPoint.Edit_API(id));
  return res.data;
};

export const enrollmentUpdate = async <T, R>(
  id: number,
  data: T
): Promise<R> => {
...
export const enrollmentTrash = async <T>(data: T): Promise<T> => {
