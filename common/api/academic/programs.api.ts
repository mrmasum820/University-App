...
export const ProgramsQueryKey = {
  ALL_List: "programsyALLList",
  List: "programsList",
...
//  api config
export const programsAllList = async <R>(params: paramsProps): Promise<R> => {
...
export const programsList = async () => {
  const res = await apiConfig.get(ApiEndPoint.LIST_API);
  return res.data;
};

export const programsCreate = async <T, R>(data: T): Promise<R> => {
  const res = await apiConfig.post<R>(ApiEndPoint.CREATE_API, data);
  return res.data;
};

export const programsEdit = async (id: number) => {
  const res = await apiConfig.get(ApiEndPoint.Edit_API(id));
  return res.data;
};

export const programsUpdate = async <T, R>(id: number, data: T): Promise<R> => {
  const res = await apiConfig.put<R>(ApiEndPoint.UPDATE_API(id), data);
  return res.data;
};

export const programsTrash = async <T>(data: T): Promise<T> => {
