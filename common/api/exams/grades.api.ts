...
export const GradesQueryKey = {
  ALL_List: "gradesALLList",
  List: "gradesList",
  Create: "gradesCreate",
  Edit: "gradesEdit",
  Update: "gradesUpdate",
...
//  api config
export const gradesAllList = async <R>(params: paramsProps): Promise<R> => {
...
export const gradesCreate = async <T, R>(data: T): Promise<R> => {
  const res = await apiConfig.post<R>(ApiEndPoint.CREATE_API, data);
  return res.data;
};

export const gradesEdit = async (id: number) => {
  const res = await apiConfig.get(ApiEndPoint.Edit_API(id));
  return res.data;
};

export const gradesUpdate = async <T, R>(id: number, data: T): Promise<R> => {
  const res = await apiConfig.put<R>(ApiEndPoint.UPDATE_API(id), data);
  return res.data;
};

export const gradesTrash = async <T>(data: T): Promise<T> => {
