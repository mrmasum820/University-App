...
export const ExamScheduleQueryKey = {
  ALL_List: "examScheduleALLList",
  List: "examScheduleList",
  Create: "examScheduleCreate",
  Edit: "examScheduleEdit",
  Update: "examScheduleUpdate",
  Delete: "examScheduleDelete",
};
// api endpoint
...
//  api config
export const examScheduleAllList = async <R>(
  params: paramsProps
): Promise<R> => {
...
export const examScheduleList = async () => {
  const res = await apiConfig.get(ApiEndPoint.LIST_API);
  return res.data;
};

export const examScheduleCreate = async <T, R>(data: T): Promise<R> => {
  const res = await apiConfig.post<R>(ApiEndPoint.CREATE_API, data);
  return res.data;
};

export const examScheduleEdit = async (id: number) => {
  const res = await apiConfig.get(ApiEndPoint.Edit_API(id));
  return res.data;
};

export const examScheduleUpdate = async <T, R>(
  id: number,
  data: T
): Promise<R> => {
...
export const examScheduleTrash = async <T>(data: T): Promise<T> => {
