...
import { AgentsQueryKey, agentsTrash } from "@/common/api";
import { ActionDropdown, OutlineButton } from "@/uikit/ui";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { Controller, SubmitHandler, useForm } from "react-hook-form";

export default function AgentsListFilter({
  table,
  ...
  const mutation = useMutation<TrashEntity, Error, TrashEntity>({
    mutationKey: [AgentsQueryKey.Delete],
    mutationFn: (value) => agentsTrash(value),
    onSettled: () => {
      refetchFn(queryClient, AgentsQueryKey.ALL_List);
