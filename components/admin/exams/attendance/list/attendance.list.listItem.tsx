import { ExamAttendanceEntity, formatDateTime } from "@/common";
...
export default function AttendanceListListItem() {
  const columns: ColumnDef<ExamAttendanceEntity>[] = [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <p>{roll_number?.number ?? 101}</p>;
      },
    },
    {
      accessorKey: "name",
...

    {
      ...
      cell: ({ row }) => {
        ...
        return <p>{attendance?.name ?? "Present"}</p>;
      },
    },
    {
