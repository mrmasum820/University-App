...
import { MarksQueryKey, marksTrash } from "@/common/api/exams/marks.api";
import { ActionDropdown, Button } from "@/uikit/ui";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { Controller, SubmitHandler, useForm } from "react-hook-form";

export default function MarksListFilter({
  table,
}: TableFilterProps<MarksEntity>) {
  ...
  const mutation = useMutation<TrashEntity, Error, TrashEntity>({
    mutationKey: [MarksQueryKey.Delete],
    mutationFn: (value) => marksTrash(value),
    onSettled: () => {
