...
import { ServiceQueryKey, serviceTrash } from "@/common/api";
import { TableFilterItemTab } from "@/common/components";
import { ActionDropdown, Button } from "@/uikit/ui";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { Controller, SubmitHandler, useForm } from "react-hook-form";

export default function ServiceListFilter({
  table,
  tableData,
}: TableFilterProps<ServiceEntity>) {
  ...
  const mutation = useMutation<TrashEntity, Error, TrashEntity>({
    mutationKey: [ServiceQueryKey.Delete],
    mutationFn: (value) => serviceTrash(value),
    onSettled: () => {
